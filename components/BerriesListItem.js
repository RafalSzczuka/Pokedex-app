...
export const BerriesListItem = ({name, url, index, isRefreshing}) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const response = await fetchDetails(url, signal);
      const imageResponse = await fetchDetails(response.item.url, signal);
  ...
  return (
    <TouchableOpacity
      onPress={() => Alert.alert(`This is ${name.toUpperCase()} berry. Yummy!`)}
      disabled={!isActive}
      key={index}
      style={[
